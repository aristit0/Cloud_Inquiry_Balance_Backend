pipeline {
    ...
    environment {
        // Git Configuration
        GIT_REPO   = 'https://github.com/aristit0/Cloud_Inquiry_Balance_Backend.git'
        GIT_BRANCH = 'main'

        // Docker Configuration
        DOCKER_IMAGE     = 'cloud-inquiry-balance-backend'
        DOCKER_CONTAINER = 'cloud-inquiry-balance-backend'
...
COPY . .
RUN CGO_ENABLED=0 GOOS=linux go build -o cloud-inquiry-balance-api .

FROM alpine:latest
RUN apk --no-cache add ca-certificates
WORKDIR /root/
COPY --from=builder /app/cloud-inquiry-balance-api .
EXPOSE 8080
CMD ["./cloud-inquiry-balance-api"]
EOF
                    fi
